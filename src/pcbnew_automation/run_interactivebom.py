...
def run_interactive_bom(pcb_file, output_dir, record=True):
    ...
    with recorded_xvfb(recording_file, **xvfb_kwargs) if record else Xvfb(**xvfb_kwargs):
        with PopenContext(['pcbnew', pcb_file], close_fds=True) as pcbnew_proc:
            ...
            xdotool(['search', '--name', 'InteractiveHtmlBom v2.3', 'windowfocus'])
            logger.info('first..')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='KiCad automated BOM runner')
    ...
    run_interactive_bom(args.kicad_pcb_file, args.output_dir, args.record)    